...
public class ImageValidateCodeGenerator implements ValidateCodeGenerator {
	...
	public ImageValidateCode generate(ServletWebRequest request) {
		ImageCodeProperties imageCodeProperties = luckyProperties.getSecurity().getCode().getImage();
		ValidateCodeParameterProperties validateCodeParameterProperties  = luckyProperties.getSecurity().getParameter().getCode();
		int width = ServletRequestUtils.getIntParameter(request.getRequest(), validateCodeParameterProperties.getImageWidthParameterName(),
				imageCodeProperties.getWidth());
		int height = ServletRequestUtils.getIntParameter(request.getRequest(), validateCodeParameterProperties.getImageHeightParameterName(),
				imageCodeProperties.getHeight());
		int length =ServletRequestUtils.getIntParameter(request.getRequest(), validateCodeParameterProperties.getImageLengthParameterName(),
				imageCodeProperties.getLength());
		int expireIn=ServletRequestUtils.getIntParameter(request.getRequest(), validateCodeParameterProperties.getImageExpireInParameterName(),
				imageCodeProperties.getExpireIn());
		String type = ServletRequestUtils.getStringParameter(request.getRequest(), validateCodeParameterProperties.getImageTypeParameterName(),
				imageCodeProperties.getImageTypeEnum().toString().toLowerCase()).toLowerCase();
		BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		Graphics g = image.getGraphics();
		setBackGround(g,width,height);
		drawRandomLine(g,width,height);
		String random = drawRandomNum((Graphics2D) g,length,type);//根据客户端传递的createTypeFlag标识生成验证码图片
		return new ImageValidateCode(image, random, expireIn);
	}
	 ...
	 */
	private String drawRandomNum(Graphics2D g,int length,String... createTypeFlag) {
		// 设置颜色
	 ...
	 * @return 随机字符
	 */
	private String createRandomChar(Graphics2D g,String baseChar,int length) {
