...
//		model.setTimeExpire("dsadsa");
    ...
    /**
     * 对支付宝手机网站支付请求的必须需要的参数进行校验 需要进行校验的参数如下：
     * <p>
    ...
    protected void paymentRequestParametersValidate(AlipayRequest alipayRequest) {
        ...
        Assert.notNull(alipayRequest, "AlipayRequest为空，无法发起支付宝手机网站支付请求");
        AlipayObject alipayObject = alipayRequest.getBizModel();
        if (isSupportAlipayObject(alipayObject.getClass())) {
            AlipayTradeWapPayModel alipayTradeWapPayModel = (AlipayTradeWapPayModel) alipayObject;
            // 商户订单号
            String out_trade_no = alipayTradeWapPayModel.getOutTradeNo();
            //产品码
            String product_code = alipayTradeWapPayModel.getProductCode();
            //订单总金额
            String total_amount = alipayTradeWapPayModel.getTotalAmount();
            //订单标题
            String subject = alipayTradeWapPayModel.getSubject();
            Assert.notNull(subject, "订单标题为空，无法发起支付宝手机网站支付请求");
            //商户订单号是否为空
            if (StringUtils.isBlank(out_trade_no)) {
                throw new IllegalArgumentException("商户订单号为空，无法发起支付宝手机网站支付请求");
            }
            //商户订单号组成是否符合支付宝规则
            System.err.println(out_trade_no.length());
            if (out_trade_no.length() > 64) {
                throw new IllegalArgumentException("商户订单号长度超过了64位，无法发起支付宝手机网站支付请求");
            }
            if (!OrderUtils.validateOrderNo(out_trade_no)) {
                throw new IllegalArgumentException("商户订单号包含字母、数字、下划线之外的字符，无法发起支付宝手机网站支付请求");
            }
            if (!StringUtils.equals(product_code, "QUICK_WAP_WAY")) {
                throw new IllegalArgumentException("传入的支付宝手机网站支付的产品码不是FAST_INSTANT_TRADE_PAY，无法发起支付宝手机网站支付请求");
            }
            try {
                double orderAmount = Double.valueOf(total_amount);
                if (orderAmount < 0.01 || orderAmount > 100000000) {
                    throw new IllegalArgumentException("传入的订单金额无法转化为double类型，无法发起支付宝手机网站支付请求");
                }
    ...
    protected boolean isSupportAlipayObject(Class cls) {
        return AlipayTradeWapPayModel.class.isAssignableFrom(cls);
    }
...
//        ((DefaultPaymentResponse) paymentResponse).setGatewayCode(alipayTradeWapPayResponse.getCode());
//        ((DefaultPaymentResponse) paymentResponse).setGatewayMessage(alipayTradeWapPayResponse.getMsg());
//        ((DefaultPaymentResponse) paymentResponse).setBusinessCode(alipayTradeWapPayResponse.getSubCode());
//        ((DefaultPaymentResponse) paymentResponse).setBusinessMessage(alipayTradeWapPayResponse.getSubMsg());
//        ((DefaultPaymentResponse) paymentResponse).setSign((String) alipayTradeWapPayResponse.getParams().get("sign"));
//        ((DefaultPaymentResponse) paymentResponse).setParameters(alipayTradeWapPayResponse.getParams());
